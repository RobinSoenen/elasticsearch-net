...
namespace Elastic.Clients.Elasticsearch.Serverless.Next;
#else
	...
	where TCollection : class, IEnumerable<TElement>
{
	public override TCollection? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		{
			return reader.ReadValue<TCollection>(options);
		}
	...
	public override void Write(Utf8JsonWriter writer, TCollection value, JsonSerializerOptions options)
	{
		switch (value)
		{
			case ICollection<TElement> { Count: 1 }: writer.WriteValue(options, value.First()); return;
			case ICollection<TElement>: writer.WriteValue(options, value); return;
			case IReadOnlyCollection<TElement> { Count: 1 }: writer.WriteValue(options, value.First()); return;
			case IReadOnlyCollection<TElement>: writer.WriteValue(options, value); return;
		}
